...
func _ready() -> void:
	$Player.position = $Player/RandomSpawn.spawn
	$Enemy.position = $Enemy/RandomSpawn.spawn
	$Tower.position = $Tower/RandomSpawn.spawn
...
## [param hintname] hint
func find_gridmap_devhints(spawntype: Gamemanager.SpawnType) -> int:
...
func _clear_gridmap_devhints(spawntype: Gamemanager.SpawnType):
	var hint = find_gridmap_devhints(spawntype)
